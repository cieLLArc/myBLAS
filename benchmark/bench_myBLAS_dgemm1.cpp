...
// 声明你的内核函数
extern void compute_fma_ymm_alignment_blocking_openMP(double *A, double *B, double *C, int m, int n, int k);
extern void compute_fma_ymm_alignment_blocking_openMP_optimized(double *A, double *B, double *C, int m, int n, int k);
extern void compute_fma_ymm_alignment_blocking_openMP_optimized_fast(double *A, double *B, double *C, int m, int n, int k);
extern void naive_col_major_dgemm(char transa, char transb, int M, int N, int K, double alpha, const double *A, int lda, const double *B, int ldb, double beta, double *C, int ldc);
...
std::vector<uint64_t> benchmark_run_fma(double *A, double *B, double *C, int N, int trials = 50)
{
...
void warmup_run_opt(double *A, double *B, double *C, int N)
{
    for (int i = 0; i < 10; ++i)
    {
        compute_fma_ymm_alignment_blocking_openMP_optimized(A, B, C, N, N, N);
    }
}

std::vector<uint64_t> benchmark_run_opt(double *A, double *B, double *C, int N, int trials = 50)
{
...
std::vector<uint64_t> benchmark_run_naive(double *A, double *B, double *C, int N, int trials = 50)
{
...
int main()
{
    ...
    for (int N : sizes)
    {
        ...
        // std::cout << "GFLOPS: fma dgemm " << std::fixed << std::setprecision(2) << avg_gflops << std::endl;
        // write_to_csv("matrix_gflops.csv", "fma dgemm", N, avg_gflops);

        // warmup_run_fma(A, B, C, N);
        // times_ns = benchmark_run_fma(A, B, C, N);
        // avg_gflops = calculate_avg_gflops(times_ns, N);
        // std::cout << "GFLOPS: fma opt fast dgemm " << std::fixed << std::setprecision(2) << avg_gflops << std::endl;
        ...
        warmup_run_opt(A, B, C, N);
        times_ns = benchmark_run_opt(A, B, C, N);
        avg_gflops = calculate_avg_gflops(times_ns, N);
        std::cout << "GFLOPS: fma dgemm " << std::fixed << std::setprecision(2) << avg_gflops << std::endl;
        write_to_csv("matrix_gflops.csv", "fma dgemm", N, avg_gflops);
